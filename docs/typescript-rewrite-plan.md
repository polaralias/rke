...
- The npm runtime exposes 43 operations through one TypeScript registry shared by CLI and MCP; documentation disposition closes a reviewed no-update delta without manufacturing canonical knowledge, and coordination cleanup-check requires exact-tip remote integration before any separate cleanup action.
