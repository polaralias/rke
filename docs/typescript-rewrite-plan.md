...
| Memory and hot-query latency | Mixed 1k/10k/50k corpus with parent peak RSS and final RSS; 50 repeated engine searches with total, mean, and p95 elapsed time |
...
- The npm runtime exposes 44 operations through one TypeScript registry shared by CLI and MCP; documentation disposition closes a reviewed no-update delta without manufacturing canonical knowledge, coordination cleanup-check requires exact-tip remote integration before any separate cleanup action, and tracker preview maps accepted non-OKF packages without provider or task-ledger mutation.
- The default deterministic suite contains 48 tests, including executable success and malformed-input failure evidence for every public operation, exact CLI/MCP registry parity, malformed workflow state, concurrent state mutation, bounded repository-scale command output, SQLite migration/corruption recovery/concurrent readers, single-refresh impact composition, strength-aware concurrent verification, stable dirty-worktree reuse and second-edit detection, scoped graph/retrieval results, source-evidence and local-cache exclusion, bounded long-line review packets, regex timeout isolation, digest-bound reviewed-evidence consumption and invalidation, repository escape, secret eviction, legacy receipt migration, Gitleaks-safe source identities, evaluator corpus discovery, clean transport behaviour, and structural validation of the pending legacy-parity scenario corpus.
...
- Mixed-language resource evidence is stored in `benchmark-1000.json`, `benchmark-10000.json`, and `benchmark-50000.json` in this directory. Each final-code run performs exactly three refreshes across cold, explicit full verification, changed-file, and 50 repeated-query phases; the one-file edit produces one hash and one parse, and parser child-process count is zero. In the latest 50,000-file run on this Windows host, cold parsing took 188.4 seconds, the hot Git check took 7.16 seconds, full content verification hashed all files without parsing and took 104.5 seconds, and a one-file edit took 7.63 seconds. Peak RSS was 547.7 MB and final RSS 64.1 MB. Fifty repeated searches took 396.6 seconds total, averaging 7.93 seconds with a 9.37-second p95; RSS did not grow monotonically. The hot Git check is a significant large-Windows-repository latency limitation even though parsing and indexing remain incremental and memory stays bounded.

| Files | 50-query total | Mean per query | p95 per query |
| ---: | ---: | ---: | ---: |
| 1,000 | 19.9 s | 398 ms | 646 ms |
| 10,000 | 75.1 s | 1.50 s | 1.59 s |
| 50,000 | 396.6 s | 7.93 s | 9.37 s |
...
- The canonical EWF 4.6.1 package contains immediate-falsifier reset, protected acceptance, compact convergence continuity, and positive/nearby-negative prompts. It also describes explicit structural scopes and binding-only knowledge impact accurately; its catalogue mirror is digest-identical apart from declared provenance metadata.
