...
export function gitDelta(root:string,base:string):CommandResult {
  ...
  for(const path of untracked){try{const absolute=join(root,path),details=lstatSync(absolute);if(details.isFile()){const content=readFileSync(absolute);if(/(^|\/)index\.md$/.test(path)&&content.includes("<!-- Generated by engineering-workflow OKF knowledge index builder. -->"))continue;identities.push(`${path}\0${content.length}\0${sha256(content)}`);}}catch{return{code:1,stdout:"",stderr:`Unable to fingerprint untracked path: ${path}`};}}
