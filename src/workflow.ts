...
export async function closureAssessment(root:string,base="HEAD"):Promise<OperationOutcome>{const state=await load(root);const change=await receiptStatus(root,base,"change-explanation.json"),documentation=await receiptStatus(root,base,"documentation-receipt.json");const validationGates=state.outstanding_gates.filter(g=>["implementation-validation","integrated-tree-validation"].includes(g)),taskGates=state.outstanding_gates.filter(g=>g==="task-reconciliation"),knowledgeGates=state.outstanding_gates.filter(g=>g.includes("knowledge"));const ready=!state.outstanding_gates.length&&!change.issues.length&&!documentation.issues.length;const lanes={change,knowledge:{...documentation,gates:knowledgeGates},validation:{status:validationGates.length?"pending":"clear",gates:validationGates},tasks:{status:taskGates.length?"pending":"clear",gates:taskGates},other:state.outstanding_gates};return outcome({result:ready?"closure-ready":"closure-blocked",ready,base,state_path:statePath(root),lanes,outstandingGates:state.outstanding_gates,state},ready?0:3);}
