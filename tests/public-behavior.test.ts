...
test("every public operation has an executable success and malformed-input failure contract",async t=>{
  ...
  const smallRoot=await mkdtemp(join(tmpdir(),"rke-public-small-"));
  t.after(async()=>{assert.ok(resolve(smallRoot).startsWith(`${resolve(tmpdir())}${sep}`));await rm(smallRoot,{recursive:true,force:true});});
  assert.equal(spawnSync("git",["init"],{cwd:smallRoot,encoding:"utf8"}).status,0);
  assert.equal(spawnSync("git",["config","user.email","tests@example.test"],{cwd:smallRoot,encoding:"utf8"}).status,0);
  assert.equal(spawnSync("git",["config","user.name","RKE Contract Tests"],{cwd:smallRoot,encoding:"utf8"}).status,0);
  await writeFile(join(smallRoot,".gitignore"),".engineering-workflow/\n");
  await writeFile(join(smallRoot,"calculator.mjs"),"export const add = (a, b) => a + b;\n");
  assert.equal(spawnSync("git",["add","."],{cwd:smallRoot,encoding:"utf8"}).status,0);
  assert.equal(spawnSync("git",["commit","-m","baseline"],{cwd:smallRoot,encoding:"utf8"}).status,0);
  await invokeOperation(smallRoot,"workflow_activate",{phase:"deliver",taskMode:"none"});
  await writeFile(join(smallRoot,"calculator.mjs"),"export const add = (a, b) => a + b;\nexport const divide = (a, b) => a / b;\n");
  await writeFile(join(smallRoot,".engineering-workflow","detail.json"),JSON.stringify({before:"Only add was exported",after:"divide is exported",why:"A second arithmetic operation was requested",causalPath:[{path:"calculator.mjs",symbol:"divide"}],verification:[]}));
  const small=await invokeOperation(smallRoot,"workflow_complete_small_change",{base:"HEAD",summary:"The calculator now exports a divide operation.",detailFile:".engineering-workflow/detail.json",reviewedPaths:["calculator.mjs"],evidence:"Reviewed the source delta; no canonical documentation surface exists."});
  assert.equal(small.exitCode,0,JSON.stringify(small.payload));observed.add("workflow_complete_small_change");
