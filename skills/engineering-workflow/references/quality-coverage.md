...
| Reliable invocation | one idempotent `activate` command; activation-time Git baseline; marker-owned Codex `AGENTS.md` routing; packaged evaluation corpus; CLI and MCP share the complete operation registry; `.githooks/pre-push` checks closure through configured `core.hooksPath` | lifecycle activation/reopen tests; project-routing merge/idempotence tests; evaluator distinguishes activation-before-edit; operation-registry and CLI/MCP parity tests; clean wheel evaluation-resource smoke; pre-push base, path-ownership, and Claude config merge tests | catalogue-only implicit selection remains host/model behaviour; Codex MCP activation is intentionally user-level and explicit |
